...
import com.hanghaemini4.jaranghalram.exceptionHandler.CommentException;
...
public class CommentService {
    ...
    public ResponseDto<?> add(Long postId, CommentRequestDto requestDto, User user) {
        Post post = postRepository.findById(postId).orElseThrow(() -> new CommentException("존재하지 않는 게시글입니다."));
    ...
    public ResponseDto<String> update(Long postId, CommentRequestDto requestDto, User user) {
        Comment comment = commentRepository.findById(postId).orElseThrow(() -> new CommentException("댓글이 존재하지 않습니다."));
    ...
    public ResponseDto<?> delete(Long postId, User user) {
        ...
        if (comment.getUser().getUserName().equals(user.getUserName())) {
            ...
            throw new CommentException("작성자만 삭제 가능");
        }
    }
}
